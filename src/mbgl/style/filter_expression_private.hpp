...
bool AnyExpression::evaluate(const Extractor& extractor) const {
	...
	const FilterExpression* FLT = expressions.data();
	const size_t FLTS = expressions.size();
	for (size_t exprIndex = 0; exprIndex < FLTS; exprIndex++) {
		const FilterExpression* FLTIDX = FLT + exprIndex;
		if (mapbox::util::apply_visitor(evaluator, *FLTIDX)) {
            return true;
        }
    }
    return false;
